...
Реализованы все пункты задания, описание реализации содержится в `README.md`

Тест: запускается сервер, который ждет подключения клиента. Далее клиент посылвает файл серверу, после чего сервер посылает клиенту другой файл. Затем и клиент, и сервер проверяют полученные файлы на равенство с оригинальными файлами. Если они идентичны, то в консоль выводится `OK!`

Скрин со статистикой по пакетам (кол-во попыток отправки, кол-во реальных отправок и кол-во пришедших пакетов):
![](images/image.png)
...
Решение находится в файле `checksum.py`, тесты в файле `test/checksum_test.py`
...
$R$ - средняя пропускная способность

$\dfrac{1}{L}$ - количество посылок между потерями

$\dfrac{MSS}{L}$ - объем передаваемых данных

$$T = \dfrac{MSS}{R \cdot L}$$

$$R = \dfrac{1.22 \cdot MSS}{RTT \cdot \sqrt{L}}$$

$$L=(\dfrac{1.22 \cdot MSS}{R \cdot RTT})^2$$

Тогда:

$$T = \dfrac{R \cdot RTT^2}{1.22^2 \cdot MSS} = T(R)$$
...
Один $RTT$ уйдет на handshake, также на первый и последний пакеты вместе уйдет $\dfrac{S}{R} + 2RTT$

1. Размер окна покроет $RTT$ третьим пакетом. Тогда первые 2 пакета будут передаваться $\dfrac{S}{R} + RTT$, третий пакет $4\dfrac{S}{R}$, четвертый пакет $8\dfrac{S}{R}$.

$$t = 4RTT + 14\dfrac{S}{R}$$

2. Размер окна покроет $RTT$ четвертым пакетом. Тогда первые 3 пакета будут передаваться $\dfrac{S}{R} + RTT$, четвертый пакет $8\dfrac{S}{R}$.

$$t = 5RTT + 11\dfrac{S}{R}$$

3. Размер окна покроет $RTT$ вторым пакетом. Тогда первый пакет будет передаваться $\dfrac{S}{R} + RTT$, остальные вместе $14\dfrac{S}{R}$
...
$$S = \dfrac{W}{2} + (1+a) \cdot \dfrac{W}{2} + \dots + (1+a)^{\log_{1+a}2}\dfrac{W}{2}=\dfrac{W \cdot (2a + 1)}{a}$$

Тогда:

$$L=\dfrac{a}{W \cdot (2a + 1)}$$
