...
#### Демонстрация работы
Файлы из примеров находятся в папке `./lab03/examples`
...
#### Решение
Рассмотрим один бит сообщения. При передаче этого бита от хоста А к хосту Б, бит ждал $\dfrac{56*8}{128*1024} с$ на хосте А при преобразовании сигнала из аналогового в цифровой, а далее $\dfrac{56*8}{1024+1024}c + 0.005 c$ уходит на передачу сигнала от хоста А к хосту Б, где сообщение необходимо преобразовать обратно в аналоговый сигнал за $\dfrac{56*8}{128*1024} с$.

Итого:

$$2*\dfrac{56*8}{128*1024} + \dfrac{56*8}{1024+1024} + 0.005 = 0,012263184c$$
...
#### Решение
$$d = 10мс + (1000/100)мс = 20 мс$$

$$a=\dfrac{N}{d}=\dfrac{10 + 1}{20мс}=0.55~пакетов/мс$$
...
#### Решение
1. Временная разница будет равна времени передачи пакета по самой медленной линии связи, то есть $\dfrac{L}{R_s}$, так как отправка второго пакета произойдет через $\dfrac{L}{R_s}$ секунд после отправки первого (сначала передача идет по линии со скоростью передачи $R_s$). Вторая линия передачи быстрее, соответственно она не увеличит временную разницу.
2. Второй пакет может находиться во входном буфере, ожидая передачи во вторую линию, так как первый пакет полностью покинет маршрутизатор за $\Delta_1= \dfrac{L}{R_s} + d_{распр} + \dfrac{L}{R_c}$, а второй доберется $\Delta_2=2*\dfrac{L}{R_s} + d_{распр}<\Delta_1$. Отсюда, можно найти минимальное $T$: $T=\Delta_1 - \Delta_2 = \dfrac{L}{R_c} - \dfrac{L}{R_s}$
